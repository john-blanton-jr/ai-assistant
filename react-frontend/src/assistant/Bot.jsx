        ...
        <div className="row">
            <div className="offset-3 col-6">
                <div className="shadow p-4 mt-4 rounded">
                    <h1 className="text-center">ChattrBox</h1>
                    <div className="mt-4 d-flex flex-column align-items-start">
                        {chatHistory.map((message, index) => (
                            <div key={index} className={`d-flex ${message.role === "user" ? 'justify-content-end w-100' : 'justify-content-start w-100'}`}>
                                <div className={`mt-1 pt-2 px-3 fs-6 bg-opacity-75 ${message.role === "user" ? 'bg-primary rounded-user' : 'bg-success rounded-assistant'} d-inline-block text-white`} style={{maxWidth: '75%'}}>
